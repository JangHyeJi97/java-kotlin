...
// 학점을 저장할 클래스로 Student를 상속받아 학생 기본정보를 가지게 됨
	int korean, history, math;	double average;
	// 점수로 학점에만 있는 정보를 저장할 멤버변수
	public Grade(String classOf, String name, int korean, int history, int math) {
		super(classOf, name);
...
//Student, Grade 클래스를 이용하여 실제 작업을 처리하는 클래스
	...
	// FirstStudents 배열에 추가할 친구정보 인스턴스를 인수로 받아와 FirstStudents배열에 저장시키는 메소드
	...
	//사용자가 입력하는 학생정보를 받아 저장시키는 메소드
		...
		int korean = 0;
		int history = 0; 
		int math = 0;//학생정보를 저장할 변수들을 선언

		Scanner sc = new Scanner(System.in);
		System.out.print("학번 : ");		classOf = sc.nextLine();
		System.out.print("이름 : ");		name = sc.nextLine();
		//성적을 나타내기 위해 필요로 하는 학생기본정보(Student클래스의 멤버변수)들을 입력받음


		if (choice == 1) {//성적 등록시
			System.out.println("국어 점수 : " + korean);		korean = sc.nextInt();
			System.out.println("국사 점수 : " + history);		history = sc.nextInt();
			System.out.println("수학 점수 : " + math);		math = sc.nextInt();
			//성적 정보 입력받음
	...
	//모든 학생들의 모든 정보를 보여주는 메소드
		for (int i = 0 ; i < numOfStudents ; i++) {
			...
			// FirstStudents[i] : FirstStudents 배열의 i인덱스에 들어있는 student형 인스턴스
			// .showAllInfo() : 안에 있는 showAllInfo() 메소드 호출
			// showAllInfo() 메소드는 하위클래스(Grade에서 오버라이딩 했기 때문에 
			// 상위클래스인 Student가 아닌 하위클래스의 메소드가 실행됨
			System.out.println();
		}
	...
	// 모든 친구들의 기본 정보를 보여주는 메소드
		for (int i = 0 ; i < numOfStudents ; i++) {
			...
			// 배열의 i인덱스에 들어있는 인스턴스가 Grade 형으로 형변환이 가능하면
	...
	// 이름으로 학생정보를 검색하여 출력하는 메소드
		Scanner sc = new Scanner(System.in);
		System.out.print("검색할 이름 : ");
		String keyword = sc.nextLine();		// 검색어(학생정보들 중 이름)

		for (int i = 0 ; i < numOfStudents ; i++) {
		// 저장된 학생정보들 만큼 루프를 돌면서 입력받은 검색어와 동일한 이름이 있는 검사 후 출력
		// 이름 : FirstStudents 배열안에 들어있는 인스턴스들의 name 멤버변수
			if (keyword.equals(FirstStudents[i].getName())) {
			// 검색어와 동일한 이름을 가진 인스턴스가 FirstStudents배열에 있으면
				FirstStudents[i].showAllInfo();
				return;
			}
		}
...
class FirstStudents3 {
	public static void main(String[] args) {
		ReportCardHandler handler = new ReportCardHandler(10);
		//  ReportCardHandler 형 인스턴스 handler를 선언 및 생성
...
// 검색결과가 없으면 "검색결과가 없습니다." 메시지 출력
			...
			switch (choice) {
				case 1: // 사용자가 입력한 메뉴번호가 1이면
					handler.addStudent(choice);	break;
					// 선택한 메뉴번호를 인수로 하여 handler인스턴스의 addStudent()메소드를 호출
				case 2:
